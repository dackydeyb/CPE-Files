...
func _input(event):
	...
	if event is InputEventMouseButton and event.pressed: # Only care about pressed events now
			...
			if event.button_index == MOUSE_BUTTON_LEFT:
				...
				if is_grass(map_pos) and not is_flag(map_pos):
					...
					if get_parent().scanners_available > 0:
						...
						get_parent().get_node("HUD").get_node("ScannerCount").text = str(get_parent().scanners_available) # Update HUD
...
func _process(_delta):
	...
	if get_parent().get_current_state() == get_parent().GameState.PLAYING and \
	   mouse_map_pos.x >= 0 and mouse_map_pos.x < COLS and mouse_map_pos.y >= 0 and mouse_map_pos.y < ROWS:
		...
		# Single cell highlighting for grass or revealed numbers
