...
func _ready():
	randomize()    
	scale = Vector2(1.25, 1.25) # for Medium
	new_game()
...
# Modified generate_numbers() with adjusted probabilities
	...
	if event is InputEventMouseButton:
		...
		if map_pos.x >= 0 and map_pos.x < COLS and map_pos.y >= 0 and map_pos.y < ROWS:
			...
			if event.button_index == MOUSE_BUTTON_LEFT:
				if event.pressed:
					# Only process left click if tile is covered by grass AND not flagged
						...
						if not Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT):
							if get_parent().first_click:
								...
								if is_mine(map_pos):
									...
									process_left_click(map_pos)
				else:  # Handle left button release for chording
					if chording:
						scan_mines(map_pos)
						chording = false
			...
			# Right click handling
				...
				if event.pressed:
					# Only process right click if tile is covered by grass
					if is_grass(map_pos):
						if not Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
							process_right_click(map_pos)
				else:  # Handle right button release for chording
					if chording:
						scan_mines(map_pos)
						chording = false
...
# Updated revert logic with dynamic changes
func try_revert_variants():
	for cell in get_used_cells(number_layer):
		...
		# Only process variant tiles (rows 2-5)
		if current_row < 2 || current_row > 5:
			continue
		
		# Calculate actual mine count
		var cnt := 0
		...
		if cnt == 0:
			...
			continue
		var r = randf()
		if current_row == 5:  # Shuffle tiles
			if r < 0.65:      # 65% stay shuffled
				continue
			elif r < 0.85:    # 20% revert to normal
				set_cell(number_layer, cell, tile_id, number_atlas[cnt - 1])
			else:             # 15% transform to another variant
				set_random_variant(cell, cnt)
		else:                # Other variants
			if r < 0.70:      # 70% revert to normal
				set_cell(number_layer, cell, tile_id, number_atlas[cnt - 1])
			elif r < 0.90:    # 10% become shuffle
				set_cell(number_layer, cell, tile_id, shuffle_atlas[cnt - 1])
			else:             # 15% random other variant
				set_random_variant(cell, cnt)

# Helper function for variant transformations
