	...
	if map_pos.x >= 0 and map_pos.x < board_cols and map_pos.y >= 0 and map_pos.y < board_rows:
		if event is InputEventMouseButton:
			if event.button_index == MOUSE_BUTTON_LEFT:
				...
				if event.pressed:
					# Only allow bomb transfer if Player2 is eligible and hasn't transferred yet
					var main = get_parent()
					if main.tile_map_player2.can_bomb_transfer and not main.tile_map_player2.has_transferred_bomb and is_grass(map_pos) and not is_mine(map_pos):
						var success: bool = main.try_transfer_bomb_to_cell(2, map_pos)
						if success:
							print("Bomb transfer activated at %s for Player 2 (to Player 1)" % [map_pos])
							main.tile_map_player2.has_transferred_bomb = true
							main.tile_map_player2.can_bomb_transfer = false
							main.mul_hud.set_mines_label_white(2)
						else:
							print("Bomb transfer failed (no available bomb to remove or other issue)")
					else:
						print("Bomb transfer not available for Player 2.")
